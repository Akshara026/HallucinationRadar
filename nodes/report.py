...
def report_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report_text = generate_report(
        query, answer, claims, verdicts, score_data, retrieval_stats
    )
...
def generate_report(
    ...
    if retrieval_stats:
        ...
        wiki_live = retrieval_stats.get("wiki_live_fetches", 0)
        wiki_cache = retrieval_stats.get("wiki_cache_hits", 0)
        wiki_fail = retrieval_stats.get("wiki_search_failures", 0)
        arxiv_live = retrieval_stats.get("arxiv_live_fetches", 0)
        arxiv_cache = retrieval_stats.get("arxiv_cache_hits", 0)
        arxiv_fail = retrieval_stats.get("arxiv_search_failures", 0)
        arxiv_disabled = retrieval_stats.get("arxiv_disabled_this_run", False)

        lines.append(f"  Wikipedia:")
        lines.append(f"    Live fetches:  {wiki_live}")
        lines.append(f"    Cache hits:    {wiki_cache}")
        lines.append(f"    Failures:      {wiki_fail}")
        lines.append(f"")
        lines.append(f"  arXiv:")
        if arxiv_disabled:
            lines.append(f"    STATUS:        DISABLED (circuit breaker)")
        lines.append(f"    Live fetches:  {arxiv_live}")
        lines.append(f"    Cache hits:    {arxiv_cache}")
        lines.append(f"    Failures:      {arxiv_fail}")
        lines.append("")
    ...
    lines.append("Evidence sourced from Wikipedia and arXiv")
    lines.append("─" * 66)
...
def get_sentence_verdict(matched_claims: List[Dict[str, Any]]) -> str:
    """Determine the verdict indicator based on worst verdict."""
...
def annotate_answer_with_worst_verdict(
    ...
    """Annotate each sentence with verification status using worst verdict."""
...
def find_all_matching_claims(
    ...
    """Find ALL claims that match a sentence."""
